...
4. 执行 `scripts/setup.sh` 下载 MaaFramework，按 [Agent runtime 构建](docs/agent-runtime.md)构建自己的 agent runtime，然后出包：
...
```bash
scripts/setup.sh   # 下载 MaaFramework 二进制；资源项目只由 CI 克隆和编译
