...
- `resource/m9a-agent-runtime-arm64-v8a.zip` — the Python agent runtime, referenced by `bundle_sha256`. It is a build artifact and is not committed.
...
# 4. record the digest in resource/m9a.toml
...
The `bundle_sha256` in `resource/m9a.toml` pins the archive, so step 4 must be repeated whenever the runtime changes. Keep the MaaFramework version of `libMaaAgentClient.so`/`libMaaAgentServer.so` aligned with the libraries vendored in `vendor/maa/android`.
