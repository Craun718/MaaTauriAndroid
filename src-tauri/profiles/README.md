...
When `interface.json` declares `agent`, configure the matching number of `[[agent.runtimes]]` entries. Each entry requires a local `bundle`, its lowercase `bundle_sha256`, the executable, the explicit list of files to mark executable, and the server command. Use `{pi}`, `{bundle}`, `{identifier}`, and `{nativeLib}` placeholders in paths, arguments, and environment values.
